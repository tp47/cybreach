...
import { BoardMatrix, MatrixGeneratorConfig, BoardSequence, BoardSequences } from '@/model'
...
class MatrixGenerator {
  ...
  private generateSequence(matrix: BoardMatrix, length: number): BoardSequence {
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (searchInRow) {
        while (matrixIndex === -1) {
          currentColumn = getRandomSeededInteger(0, matrixSize - 1, this.seededRNG)
          ...
          if (i < length - 1 && this.getValuesLeftInColumn(matrixCopy, currentColumn) < 2) {
            matrixIndex = -1
          }
        }
        searchInRow = false
        ...
        while (matrixIndex === -1) {
          currentRow = getRandomSeededInteger(0, matrixSize - 1, this.seededRNG)
          matrixIndex = this.getMatrixIndex(matrixSize, currentRow, currentColumn)
          ...
          if (i < length - 1 && this.getValuesLeftInRow(matrixCopy, currentRow) < 2) {
            matrixIndex = -1
          }
        }
        searchInRow = true
